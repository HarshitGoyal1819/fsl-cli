#!/usr/bin/env bash
# =============================================================================
# add_to_path.sh — add /usr/local/bin to PATH for fsl-cli
...
INSTALL_PATH="/usr/local/bin/fsl-cli"
PATH_LINE='export PATH="/usr/local/bin:$PATH"'
COMMENT='# Added by fsl-cli'
...
case "${SHELL:-}" in
    ...
    */fish)
        ...
        grep -q "fsl-cli" "$FISH" 2>/dev/null || {
            echo "" >> "$FISH"
            echo "$COMMENT" >> "$FISH"
...
if [ -d "/etc/paths.d" ] && [ ! -f "/etc/paths.d/fsl-cli" ]; then
    echo "/usr/local/bin" | sudo tee /etc/paths.d/fsl-cli > /dev/null 2>&1 || true
fi

echo ""
echo "✓  Done. Restart your terminal, then run:  fsl-cli"
