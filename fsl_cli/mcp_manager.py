"""MCP (Model Context Protocol) configuration manager for fsl-cli.

Manages ~/.fsl-cli/mcp.json — compatible with Kiro IDE format.
...
Full schema (all fields optional except name + one of url/command):
{
  "mcpServers": {
    "<name>": {
      ...
      // Style B — typed auth block (fsl-cli extended):
      "auth": {
...
# Also look for a project-level mcp.json in .fsl-cli/ relative to cwd
_PROJECT_MCP_RELATIVE = ".fsl-cli/mcp.json"
...
def add_server(
    ...
    *,
    ...
    scope_label = f"project ({path})" if scope_level == "project" else "global (~/.fsl-cli/mcp.json)"
