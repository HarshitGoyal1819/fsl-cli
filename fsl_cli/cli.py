...
from fsl_cli.ollama_launcher import ensure_ollama_running, is_ollama_running, stop_ollama
from fsl_cli.tools import ALL_TOOLS
from fsl_cli.ui import FslConsole

app = typer.Typer(
    name="fsl-cli",
    help="fsl-cli — a local-first agentic AI assistant.",
...
def _add_custom_model(console: "FslConsole", session: "Session") -> None:
    """Interactive wizard to add a custom premium model."""
    from fsl_cli.config import add_custom_model, save_api_key_for_env, set_active_model

    console.print("\n[fsl.brand]  ── Add a custom model ──[/fsl.brand]\n")
    console.print("[fsl.dim]  Add any model from a provider fsl-cli supports.[/fsl.dim]\n")

    # ── Provider ─────────────────────────────────────────────────────────
    console.print("  Provider:")
    console.print("  [fsl.accent]1[/fsl.accent]  OpenAI-compatible  (OpenAI, DeepSeek, Together, Groq, OpenRouter, local vLLM…)")
    console.print("  [fsl.accent]2[/fsl.accent]  Anthropic  (Claude)")
    console.print("  [fsl.accent]3[/fsl.accent]  Google  (Gemini)")
    ...
    # ── Model id ─────────────────────────────────────────────────────────
    console.print("\n[fsl.dim]  Exact model name from the provider "
                  "(e.g. gpt-4o, claude-opus-4-8, deepseek-v4-pro, gemini-2.0-flash):[/fsl.dim]")
    ...
    # ── Friendly label ───────────────────────────────────────────────────
    console.print("[fsl.dim]  Display label (press Enter to use the model name):[/fsl.dim]")
    ...
    # ── API key ──────────────────────────────────────────────────────────
    from fsl_cli.config import get_api_key_for_env
    if not get_api_key_for_env(key_env):
        console.print(f"\n[fsl.dim]  Paste the API key for {key_env} and press Enter:[/fsl.dim]")
    ...
    # ── Offer to activate ────────────────────────────────────────────────
    console.print("\n[fsl.dim]  Make this the active model now? (Y/n):[/fsl.dim]")
...
def _read_secret(console: "FslConsole", label: str = "  › ") -> str:
...
def _mcp_guided_setup(console: "FslConsole") -> None:
    """Interactive step-by-step wizard for adding a new MCP server."""
    import os
    from fsl_cli.mcp_manager import add_server
    from fsl_cli.mcp_client import test_connection

    console.print("\n[fsl.brand]  ── Add MCP Server ──[/fsl.brand]\n")

    # ── Step 1: Name ────────────────────────────────────────────────────
    console.print("[fsl.dim]  Step 1/5 — Server name (e.g. celonis, stripe, github):[/fsl.dim]")
    ...
    console.print("\n[fsl.dim]  Step 2/5 — Server type:[/fsl.dim]")
    console.print("  [fsl.accent]1[/fsl.accent]  HTTP / Remote URL  (Celonis, Stripe API, any web service)")
    console.print("  [fsl.accent]2[/fsl.accent]  stdio / Local process  (uvx, npx, python -m ...)")
    ...
    if transport_choice == "2":
        # stdio
        console.print("\n[fsl.dim]  Command to run (e.g. uvx):[/fsl.dim]")
        command = console.prompt_raw("  command › ").strip()
        if not command:
            console.print_warning("Cancelled.")
            return
        console.print("[fsl.dim]  Arguments (space-separated, e.g. @stripe/agent-toolkit@latest):[/fsl.dim]")
        args_raw = console.prompt_raw("  args › ").strip()
        args_list = args_raw.split() if args_raw else []
        console.print("[fsl.dim]  Env vars (KEY=VALUE pairs, comma-separated, or leave blank):[/fsl.dim]")
        ...
        # HTTP
        console.print("\n[fsl.dim]  Server URL:[/fsl.dim]")
        url = console.prompt_raw("  url › ").strip()
        if not url:
            console.print_warning("Cancelled.")
            return
        console.print("[fsl.dim]  Transport protocol:[/fsl.dim]")
        console.print("  [fsl.accent]1[/fsl.accent]  auto  — try Streamable HTTP first, fall back to SSE (recommended)")
        console.print("  [fsl.accent]2[/fsl.accent]  sse   — legacy SSE (2024-11-05) — use for Celonis and older servers")
        console.print("  [fsl.accent]3[/fsl.accent]  streamable-http — modern (2025-03-26+)")
        tp = console.prompt_raw("  protocol [1/2/3] › ").strip()
        transport = {"2": "sse", "3": "streamable-http"}.get(tp, "auto")

    # ── Step 3: Auth type ────────────────────────────────────────────────
    console.print("\n[fsl.dim]  Step 3/5 — Authentication:[/fsl.dim]")
    console.print("  [fsl.accent]1[/fsl.accent]  none           — no auth")
    console.print("  [fsl.accent]2[/fsl.accent]  bearer         — Bearer token (most common for SaaS)")
    console.print("  [fsl.accent]3[/fsl.accent]  api_key        — Custom header API key")
    console.print("  [fsl.accent]4[/fsl.accent]  oauth2_token   — Pre-obtained OAuth2 access token")
    console.print("  [fsl.accent]5[/fsl.accent]  oauth2_client  — OAuth2 Client Credentials (machine-to-machine)")
    console.print("  [fsl.accent]6[/fsl.accent]  oauth2_pkce    — OAuth2 Authorization Code + PKCE (opens browser)")
    ...
    if auth_type == "bearer":
        console.print("[fsl.dim]  Bearer token (masked):[/fsl.dim]")
        token = _read_secret(console, "  token › ").strip()

    elif auth_type == "api_key":
        console.print("[fsl.dim]  Header name (default: X-API-Key):[/fsl.dim]")
        api_key_header = console.prompt_raw("  header › ").strip() or "X-API-Key"
        console.print("[fsl.dim]  API key value (masked):[/fsl.dim]")
        api_key = _read_secret(console, "  key › ").strip()

    elif auth_type == "oauth2_token":
        console.print("[fsl.dim]  Access token (masked):[/fsl.dim]")
        token = _read_secret(console, "  access_token › ").strip()
        console.print("[fsl.dim]  Refresh token (optional, press Enter to skip):[/fsl.dim]")
        refresh_tok = _read_secret(console, "  refresh_token › ").strip() or None

    elif auth_type == "oauth2_client":
        console.print("[fsl.dim]  Token URL (e.g. https://login.example.com/token):[/fsl.dim]")
        tok_url = console.prompt_raw("  token_url › ").strip()
        console.print("[fsl.dim]  Client ID:[/fsl.dim]")
        client_id = console.prompt_raw("  client_id › ").strip()
        console.print("[fsl.dim]  Client secret (masked, optional):[/fsl.dim]")
        client_secret = _read_secret(console, "  client_secret › ").strip() or None
        console.print("[fsl.dim]  Scope (optional, e.g. read write):[/fsl.dim]")
        scope = console.prompt_raw("  scope › ").strip() or ""

    elif auth_type == "oauth2_pkce":
        console.print("[fsl.dim]  Authorization URL (e.g. https://auth.example.com/authorize):[/fsl.dim]")
        auth_url = console.prompt_raw("  auth_url › ").strip()
        console.print("[fsl.dim]  Token URL:[/fsl.dim]")
        tok_url = console.prompt_raw("  token_url › ").strip()
        console.print("[fsl.dim]  Client ID:[/fsl.dim]")
        client_id = console.prompt_raw("  client_id › ").strip()
        console.print("[fsl.dim]  Scope (optional):[/fsl.dim]")
        scope = console.prompt_raw("  scope › ").strip() or ""

    # ── Step 4: Scope ────────────────────────────────────────────────────
    console.print("\n[fsl.dim]  Step 4/5 — Config scope:[/fsl.dim]")
    console.print("  [fsl.accent]1[/fsl.accent]  global  — available in all projects (~/.fsl-cli/mcp.json)")
    console.print("  [fsl.accent]2[/fsl.accent]  project — this project only (.fsl-cli/mcp.json in cwd)")
    scope_choice = console.prompt_raw("  scope [1/2] › ").strip()
    scope_level = "project" if scope_choice == "2" else "global"

    # ── Step 5: Save + test ───────────────────────────────────────────────
    console.print("\n[fsl.dim]  Step 5/5 — Saving and testing connection…[/fsl.dim]")
    ...
    # Test connection
    from fsl_cli.mcp_manager import get_server
    cfg = get_server(name, cwd=os.getcwd())
    if cfg:
        console.print("[fsl.dim]  Testing connection…[/fsl.dim]")
        connected, conn_msg, tools = test_connection(cfg)
        if connected:
            console.print_success(f"Connected! {conn_msg}")
            if tools:
                console.print(f"  [fsl.dim]Available tools:[/fsl.dim]")
                for t in tools[:10]:
                    console.print(f"  [fsl.accent]  • {t.get('name','?')}[/fsl.accent]  [fsl.dim]{t.get('description','')[:60]}[/fsl.dim]")
                if len(tools) > 10:
                    console.print(f"  [fsl.dim]  … +{len(tools)-10} more[/fsl.dim]")
        else:
            console.print_warning(f"Connection test failed: {conn_msg}")
            console.print("[fsl.dim]  Server saved — fix URL/auth and run: /mcp test " + name + "[/fsl.dim]")

    console.print(f"\n[fsl.dim]  Done. Use  /mcp list  to see all servers.[/fsl.dim]\n")
...
def run_model_switcher(console: FslConsole) -> bool:
    """Interactive model selection. Returns True if model was changed."""
    from fsl_cli.config import get_all_models, get_active_model_id, get_custom_models
    all_models = get_all_models()
    active_id = get_active_model_id()
    custom_ids = {m["id"] for m in get_custom_models()}

    console.print("")
    console.print("[fsl.brand]  ── Select a model ──[/fsl.brand]\n")

    pulled = _get_pulled_ollama_models()

    console.print("[fsl.success]  FREE  (runs locally via Ollama)[/fsl.success]")
    for i, m in enumerate(all_models, start=1):
        if not m["needs_key"]:
            ...
            color = "fsl.tool_ok" if is_pulled else "fsl.warning"
            console.print(
                f"  [fsl.accent]{i:>2}.[/fsl.accent] [fsl.accent]{m['label']}[/fsl.accent]"
                f"[{color}]{active_tag}{dl_tag}[/{color}]"
            )

    console.print("")
    console.print("[fsl.warning]  PREMIUM  (API key required)[/fsl.warning]")
    for i, m in enumerate(all_models, start=1):
        if m["needs_key"]:
            active_tag = " ◀ active" if m["id"] == active_id else ""
            custom_tag = " [custom]" if m["id"] in custom_ids else ""
            color = "fsl.tool_ok" if active_tag else "fsl.dim"
            console.print(
                f"  [fsl.accent]{i:>2}.[/fsl.accent] [fsl.accent]{m['label']}[/fsl.accent]"
                f"[fsl.dim]{custom_tag}[/fsl.dim][{color}]{active_tag}[/{color}]"
            )

    console.print("")
    console.print(f"[fsl.dim]  Enter a number (1–{len(all_models)}) or press Enter to cancel:[/fsl.dim]")
    ...
    if selected["needs_key"]:
        ...
        if existing:
            ...
            console.print(f"[fsl.dim]  Stored key for {key_env}: {masked}[/fsl.dim]")
            console.print("[fsl.dim]  Use stored key? (Enter = yes,  n = enter new key):[/fsl.dim]")
        ...
        if existing:
            ...
            console.print(
                f"\n  [fsl.info]Get your API key →[/fsl.info] "
                f"[fsl.accent]{selected.get('key_url','')}[/fsl.accent]"
            )
            console.print("[fsl.dim]  Paste your API key and press Enter:[/fsl.dim]")
            key = console.prompt_raw("  key › ").strip()
            if not key:
        ...
        if selected["provider"] == "ollama":
            ...
            if not is_pulled:
                console.print(
                    f"[fsl.warning]⚠  Model not downloaded yet. Run this first:[/fsl.warning]\n"
                    f"  [fsl.accent]{selected['pull_cmd']}[/fsl.accent]"
                )
                console.print("[fsl.dim]  Switch anyway? (y/N):[/fsl.dim]")
                try:
...
class SlashCommands:
    ...
    def __init__(self, console: FslConsole, session: "Session") -> None:
    ...
    def handle(self, text: str) -> bool:
        ...
        if cmd == "/setup":
            ...
            from fsl_cli.bootstrap import run_bootstrap
            try:
        ...
        if cmd == "/removemodel" or cmd == "/remove-model":
            from fsl_cli.config import get_custom_models, remove_custom_model
            customs = get_custom_models()
            if not customs:
                self.console.print_info("No custom models to remove.")
                return True
            self.console.print("\n[fsl.brand]  Custom models:[/fsl.brand]")
            for i, m in enumerate(customs, 1):
                self.console.print(f"  [fsl.accent]{i}[/fsl.accent]  {m['model']}  [fsl.dim]({m['provider']})[/fsl.dim]")
            self.console.print("[fsl.dim]  Enter number to remove, or Enter to cancel:[/fsl.dim]")
        ...
        if cmd == "/pull":
            from fsl_cli.bootstrap import pull_model, _STARTER_MODELS, _ollama_path
            ...
            if len(parts) >= 2:
                ...
                self.console.print("\n[fsl.brand]  Select a model to download:[/fsl.brand]\n")
                for i, (name, size, desc) in enumerate(_STARTER_MODELS, 1):
                    self.console.print(
                        f"  [fsl.accent]{i}[/fsl.accent]  {name:<20} "
                        f"[fsl.dim]({size}) — {desc}[/fsl.dim]"
                    )
                self.console.print("  [fsl.accent]c[/fsl.accent]  cancel")
                self.console.print(
                    f"\n[fsl.dim]  Enter a number (1–{len(_STARTER_MODELS)}), "
                    f"or type any Ollama model name:[/fsl.dim]"
                )
            ...
            if pull_model(model, self.console):
                ...
                from fsl_cli.config import set_active_model
        ...
        if cmd == "/key" or cmd == "/keys":
            ...
            if sub == "list":
                ...
                self.console.print("\n[fsl.brand]  Stored API keys[/fsl.brand]")
                self.console.print(f"[fsl.dim]  (config: ~/.fsl-cli/config.json)[/fsl.dim]\n")
                ...
                for m in MODELS:
                    if m.get("needs_key") and m.get("key_env"):
                        ...
                        if masked:
                            self.console.print(f"  [fsl.success]✓[/fsl.success] {env:<20} {masked}")
                        else:
                            self.console.print(f"  [fsl.dim]✗ {env:<20} (not set)[/fsl.dim]")
                self.console.print(
                    "\n[fsl.dim]  /key set <ENV_NAME>     add or replace a key\n"
                    "  /key remove <ENV_NAME>  delete a key[/fsl.dim]\n"
                )
                return True
                ...
                if not env_name:
                    ...
                    return True
                self.console.print(f"[fsl.dim]  Paste your key for {env_name} and press Enter:[/fsl.dim]")
        ...
        if cmd == "/mcp":
            ...
            if sub == "new":
                ...
                from fsl_cli.mcp_manager import list_servers, export_kiro_format
                import os
                ...
                if not servers:
                    ...
                    self.console.print(
                        f"\n[fsl.brand]  MCP Servers ({len(servers)}):[/fsl.brand]"
                    )
                    for s in servers:
                        status = "[fsl.error]disabled[/fsl.error]" if s.get("disabled") else "[fsl.success]enabled[/fsl.success]"
                        kind = s.get("type", "?")
                        loc = s.get("url", s.get("command", ""))[:55]
                        self.console.print(
                            f"  [fsl.accent]{s['name']:<20}[/fsl.accent] "
                            f"{status}  {kind}  auth={s.get('auth','none')}  {loc}"
                        )
                return True

            elif sub == "json":
                from fsl_cli.mcp_manager import export_kiro_format
                import os
                ...
                if not cfg:
                    ...
                    if ok:
                        ...
                        for t in tools[:8]:
                            self.console.print(f"  [fsl.dim]  • {t.get('name', '?')}[/fsl.dim]")
                        if len(tools) > 8:
                            self.console.print(f"  [fsl.dim]  … +{len(tools)-8} more[/fsl.dim]")
                    else:
                ...
                if not server_name:
                    ...
                    return True
                from fsl_cli.mcp_manager import remove_server
                import os
        ...
        if cmd == "/memory":
            from fsl_cli.memory import format_memory_for_display, clear_memory_category
            parts = cmd_raw.split(maxsplit=1)
            if len(parts) > 1 and parts[1].startswith("clear"):
                ...
                from fsl_cli.memory import format_memory_for_display
                self.console.print(f"[fsl.brand]  Memory:[/fsl.brand]\n{format_memory_for_display()}")
            return True
...
class Session:
    ...
    def run_turn(self, user_input: str, console: FslConsole) -> None:
        import time
        ...
        if self.stream:
            ...
            with console._console.status(
                "  Thinking…", spinner="dots", spinner_style="fsl.accent",
            ):
...
def main(
    ...
    """Start an interactive fsl-cli session or run a one-shot prompt."""
    ...
    console = FslConsole(quiet=quiet)
    info = get_active_model_info()
    ...
    from fsl_cli.bootstrap import is_first_run, run_bootstrap
    if is_first_run():
        try:
    ...
    if session.missing_key() and session.build_error:
        ...
        for i, ln in enumerate(session.build_error.split("\n")):
            prefix = "[fsl.warning]⚠  " if i == 0 else "[fsl.dim]"
            suffix = "[/fsl.warning]" if i == 0 else "[/fsl.dim]"
    ...
    try:
        from fsl_cli.memory import load_memory, _save as _mem_save
        mem = load_memory()
