"""First-run bootstrap for fsl-cli.
...
Runs automatically the first time fsl-cli starts (tracked via a marker file).
"""
...
from fsl_cli.config import CONFIG_DIR
        ...
        if system == "Darwin":
            ...
            try:
                ...
                console.print_warning(
                    ...
                    "Please install Ollama from https://ollama.com/download then restart fsl-cli."
                )
                return False
    ...
    try:
        ...
        if rich_console is not None:
            ...
            with Live(console=rich_console, refresh_per_second=12, transient=True) as live:
                while True:
                    ...
                    if ch == "\r":
                        ...
                        buf = ""
                        if current:
                            live.update(Text(f"  {current}", style="fsl.dim"))
                    elif ch == "\n":
                        current = buf.strip()
                        buf = ""
                        if current:
            ...
            for line in proc.stdout:
                line = line.strip()
                if line:
                    console.print(f"  [fsl.dim]{line}[/fsl.dim]")
...
def run_bootstrap(console) -> None:
    ...
    console.print("")
    console.print("[fsl.brand]  ── First-time setup ──[/fsl.brand]\n")
    console.print(
        "[fsl.dim]  fsl-cli needs Ollama to run free local models.\n"
        "  (You can skip this and use only cloud models with an API key.)[/fsl.dim]\n"
    )

    # 1. Ollama
    if not ollama_installed():
        console.print("[fsl.dim]  Install Ollama now? (Y/n):[/fsl.dim]")
    ...
    # 2. Starter model
    console.print("\n[fsl.dim]  Download a starter model?[/fsl.dim]")
    for i, (name, size, desc) in enumerate(_STARTER_MODELS, 1):
        console.print(f"  [fsl.accent]{i}[/fsl.accent]  {name:<20} ({size})  — {desc}")
    console.print("  [fsl.accent]s[/fsl.accent]  skip (I'll use cloud models or pull later)")
    ...
    if choice.isdigit() and 1 <= int(choice) <= len(_STARTER_MODELS):
        ...
        from fsl_cli.ollama_launcher import ensure_ollama_running
    ...
    mark_bootstrapped()
    console.print("\n[fsl.success]  Setup complete! You're ready to go.[/fsl.success]\n")
