...
from fsl_cli.agent.prompts import build_system_prompt
from fsl_cli.agent.state import AgentState
...
class AgentConfig:
    ...
    def from_active_config(cls, cwd: str | None = None) -> "AgentConfig":
        from fsl_cli.config import get_active_model_info
...
def _build_llm(config: AgentConfig):
    ...
    if config.provider == "ollama":
        ...
        # passing it triggers a UserWarning on every call. Only send temperature
        # for older 1.x/2.x models that still accept it.
