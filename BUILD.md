...
|---|---|---|
| fsl-cli app | ✅ Yes | PyInstaller binary |
...
| Admin/sudo password | ❌ Never | Per-user install: `~/.local/bin` (mac), `%LOCALAPPDATA%\Programs\fsl-cli` (win) |
...
would make the installer 12GB+. Instead, the first time a user runs `fsl-cli`,
...
```bash
installer -pkg dist/fsl-cli-0.1.0-macos-universal.pkg -target CurrentUserHomeDirectory
```
...
codesign --force --options runtime --sign "Developer ID Application: Your Name (TEAMID)" \
    dist/pkg_root/usr/local/bin/fsl-cli

# Sign the pkg
productsign --sign "Developer ID Installer: Your Name (TEAMID)" \
    dist/fsl-cli-0.1.0-macos-universal.pkg \
    dist/fsl-cli-0.1.0-macos-universal-signed.pkg

# Notarize
xcrun notarytool submit dist/fsl-cli-0.1.0-macos-universal-signed.pkg \
...
**Install it:**
- Double-click the `.exe`, or silently: `.\fsl-cli-0.1.0-windows-x86_64-setup.exe /S`
...
```bash
# macOS
./dist/arm64/fsl-cli          # or dist/x86_64/fsl-cli
...
`fsl_cli.spec`, or add the specific module to `hiddenimports`.
...
right-click → Open, or run `xattr -d com.apple.quarantine /usr/local/bin/fsl-cli`.
