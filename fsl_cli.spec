...
# PyInstaller spec for fsl-cli — bundles Python + all dependencies.
#
# Build on macOS  → dist/fsl-cli   (then wrapped into .pkg)
# Build on Windows → dist/fsl-cli.exe (then wrapped into NSIS installer)
...
hiddenimports += collect_submodules("fsl_cli")
hiddenimports += [
...
exe = EXE(
    ...
    # target_arch is read from the FSLCLI_TARGET_ARCH env var so build scripts
    ...
    target_arch=(
        None if os.environ.get("FSLCLI_TARGET_ARCH", "native") in ("", "native")
        else os.environ["FSLCLI_TARGET_ARCH"]
    ),
    codesign_identity=None,
    entitlements_file=None,
    icon=(
        str(ROOT / "packaging" / "macos" / "resources" / "fsl-cli.icns")
        if IS_MAC and (ROOT / "packaging" / "macos" / "resources" / "fsl-cli.icns").exists()
        else (
            str(ROOT / "packaging" / "windows" / "fsl-cli.ico")
            if IS_WIN and (ROOT / "packaging" / "windows" / "fsl-cli.ico").exists()
            else None
        )
    ),
)
