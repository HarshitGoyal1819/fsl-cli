...
fsl-cli requires a model that supports **tool / function calling**. Good options:
...
```bash
sudo installer -pkg fsl-cli-0.1.0-macos-universal.pkg -target /
```
Then open a new terminal and type `fsl-cli`.

**Windows** (x86_64):
```
Double-click fsl-cli-0.1.0-windows-x86_64-setup.exe
```
Or silent install:
```powershell
.\fsl-cli-0.1.0-windows-x86_64-setup.exe /S
```
Then open a new CMD or PowerShell and type `fsl-cli`.
...
The `fsl-cli` command is now available in your shell.
...
> fsl-cli detects whether Ollama is running at startup. If it isn't, it will
...
```bash
fsl-cli "add type hints to all functions in src/utils.py"
```
...
fsl-cli --model llama3.1:8b

# Start in a specific project directory
fsl-cli --cwd ~/projects/my-app

# One-shot: create a new file
fsl-cli "create a FastAPI hello-world app in main.py"

# One-shot: run tests and fix failures
fsl-cli "run pytest and fix any failing tests"

# Disable streaming (useful for piping output)
fsl-cli --no-stream "explain what this project does"

# Research a third-party tool
fsl-cli "search the Celonis docs and explain how PQL FILTER works"
```
...
# → dist/fsl-cli-0.1.0-macos-universal.pkg
```
...
# → dist\fsl-cli-0.1.0-windows-x86_64-setup.exe
```
...
|---|---|---|
| `FSL_DEBUG` | unset | Set to any value to print full tracebacks on errors |

### Changing the default model

Pass `--model` on every invocation, or open `fsl_cli/cli.py` and change the option default:
...
# Type-check
mypy fsl_cli/ --ignore-missing-imports

# Run directly without installing
python -m fsl_cli.cli
```
...
Ollama isn't running. fsl-cli will try to start it automatically, or you can run `ollama serve` manually.
...
**Commands time out**
The default shell timeout is 60 seconds. For long-running builds you can increase `_TIMEOUT` in `fsl_cli/tools/shell_tools.py`.
